...
> Repository development instructions for **authentication** (Service).
...
src/adapter, admin, analytics, authentication, config, core, handlers, health, observability, prisma, security
...
@omnixys/cache-ts, context-ts, contracts-ts, graphql-ts, grpc-ts, http-ts, kafka-ts, logger-ts, observability-ts, security-ts
...
node --test __tests__/unit/*.test.mjs and __tests__/integration/*.test.mjs; vitest for e2e (infrastructure-dependent)
