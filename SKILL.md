<!-- repository: services/authentication | kind: SERVICE | stack: nestjs -->

# authentication — Skill: Service Development

> Workflow for authentication (services/authentication). Execute this workflow before, during, and
...
- API: GraphQL (NestJS Apollo Federation) via Fastify
- Messaging: Kafka (kafkajs + @omnixys/kafka-ts)
- Tests: node --test __tests__/unit/*.test.mjs and __tests__/integration/*.test.mjs; vitest for e2e (infrastructure-dependent)
...
- Identify the affected bounded context within `src/adapter, admin, analytics, authentication, config, core, handlers, health, observability, prisma, security`.
...
- Keep tenant isolation intact (`Security-sensitive: token lifecycle, refresh rotation, tenant/principal binding.`).
...
FAILURE`, or `NOT RUN` (with a reason). Never convert `NOT RUN` into `PASS`.
  ...
  - `pnpm run test:e2e (vitest, infrastructure-dependent)`
  - `pnpm build`
  - `pnpm test`
