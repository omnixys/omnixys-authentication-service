...
test('Console login and cluster health monitoring', async ({ page }) => {
  ...
  await expect(
    page.locator('h3:has-text("Management components")'),
  ...
  await expect(
    page.locator('h3:has-text("Automation components")'),
  ).toBeVisible({ timeout: 30000 });

  // Verify orchestration automation components are healthy
  await expect(page.locator('h6:has-text("Zeebe")')).toBeVisible({
    timeout: 15000,
  });
  await expect(page.locator('h6:has-text("Tasklist")')).toBeVisible({
    timeout: 15000,
  });
  await expect(page.locator('h6:has-text("Operate")')).toBeVisible({
    timeout: 15000,
  });
  await expect(page.locator('h6:has-text("Optimize")')).toBeVisible({
    timeout: 15000,
  });

  // Verify all automation components show healthy status
  const automationHealthyCount = await page
    .locator('span:has-text("Healthy")')
    .count();
  ...
  await expect(
    page.locator('text=/error|failed|invalid|incorrect/i'),
  ).not.toBeVisible();
